  ...
  fPhysiWorld(NULL)
{
  ...
  fGenericTargetColourVec = G4ThreeVector(0.8,0.0,0.2);
  ...
  fSceptar2StructureMaterial = (G4String) "Mylar";
  fSceptar2BuildStruc = (G4bool) true;
  fSceptar2BuildSipm = (G4bool) true;
}
...
void DetectorConstruction::SetGenericTargetColour( G4ThreeVector vec )
{
  DetectorConstruction::fGenericTargetColourVec = vec;
  // fGenericTargetColour = G4Colour(fGenericTargetColourVec.x(),fGenericTargetColourVec.y(),fGenericTargetColourVec.z());
}

void DetectorConstruction::SetGenericTarget()
{

  G4cout << " @#$@#%^@#$ SetGenericTarget()" << G4endl;
  ...
  if( fSetGenericTargetMaterial ) {
    if( fSetGenericTargetDimensions ) {
      if( fSetGenericTargetPosition ) {
        ...
        pApparatusGenericTarget->Build(name, vecX, vecY, vecZ, fGenericTargetColourVec);
        G4RotationMatrix* rotate = new G4RotationMatrix;
        pApparatusGenericTarget->PlaceApparatus(fLogicWorld, fGenericTargetPosition, rotate);

      } else {
        G4cout << " ----> Generic target position undefined." << G4endl;
      }
    } else {
      G4cout << " ----> Generic target dimensions undefined." << G4endl;
    }
  } else {
    G4cout << " ----> Generic target material undefined." << G4endl;
  }
}
...
void DetectorConstruction::AddDetectionSystemSceptar2(G4int ndet) {
  if(fLogicWorld == NULL) {
    Construct();
  }

  DetectionSystemSceptar2* pSceptar2 = new DetectionSystemSceptar2();
  pSceptar2->SetConfig(ndet,fSceptar2BuildStruc,fSceptar2BuildSipm);
  pSceptar2->SetStructureMaterial(fSceptar2StructureMaterial);
  pSceptar2->SetStructureThickness(fSceptar2StructureThickness);
  pSceptar2->BuildDetector();
  pSceptar2->PlaceDetector(fLogicWorld);
  pSceptar2->Debrief();
}
void DetectorConstruction::SetSceptar2StructureMaterial(G4String material) {
  fSceptar2StructureMaterial = material;
}
void DetectorConstruction::SetSceptar2StructureThickness(G4double thickness) {
  fSceptar2StructureThickness = thickness;
}
void DetectorConstruction::SetSceptar2BuildStructure(G4bool build) {
  fSceptar2BuildStruc = build;
}
void DetectorConstruction::SetSceptar2BuildSipm(G4bool build) {
  fSceptar2BuildSipm = build;
}
